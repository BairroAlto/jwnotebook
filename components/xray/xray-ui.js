...
export const XRayUI = {
    ...
    abrirLeituraCompleta: async (item) => {
        ...
        if (await bloquearConteudoHistorico(item.bridge, display)) return;
        ...
        try {
            const p = (item.bridge.sigla === 'mwb') ? 'mwb' : (item.bridge.sigla === 'g' ? 'g' : (item.bridge.contexto === 'livro' ? 'livros' : 'w'));
