...
import { bloquearConteudoHistorico } from '../billing/historical-content.js';

let acessoPublicacoesHistoricas = null;

async function podeAcederPublicacoesHistoricas() {
    if (acessoPublicacoesHistoricas !== null) return acessoPublicacoesHistoricas;
    try {
        acessoPublicacoesHistoricas = await obterAcessoFerramenta(window.auth, 'publicacoes_historicas');
    } catch (erro) {
        console.error('[LISTS] Não foi possível verificar o acesso às publicações históricas:', erro);
        acessoPublicacoesHistoricas = false;
    }
    return acessoPublicacoesHistoricas;
}

function mostrarBloqueioPublicacoesHistoricas(container) {
    const aviso = document.createElement('div');
    aviso.style.cssText = 'margin:10px; padding:12px; border:1px solid rgba(244,114,182,.45); border-radius:8px; color:#f9a8d4; font-size:11px;';
    aviso.textContent = 'As edições de A Sentinela e Despertai! anteriores a 2000 estão disponíveis nos planos Premium e Premium Plus.';
    (container.querySelector('#livros-scroll') || container).prepend(aviso);
}
...
function renderizarCategorias(container) {
    ...
    container.innerHTML = `
        ...
        <div id="livros-scroll" style="flex: 1; overflow-y: auto; padding: 10px 0;">
            ...
            <div class="menu-item-list" id="cat-despertai"><i class="fa-solid fa-newspaper"></i> Despertai!</div>
    ...
    document.getElementById('cat-despertai').onclick = () => escolherAno('despertai', container);
...
async function escolherAno(pasta, container) {
    const ePublicacaoHistorica = ['sentinelas', 'despertai'].includes(pasta);
    const permiteHistoricas = ePublicacaoHistorica
        ? await podeAcederPublicacoesHistoricas()
        : true;
    const nomes = { 'sentinelas': 'A Sentinela', 'mwb': 'Manual de Atividades', 'multimedia': 'Multimédia' };
    nomes.despertai = 'Despertai!';
    container.innerHTML = `
        ...
        <div id="livros-scroll" style="flex:1; overflow-y:auto; display: grid; grid-template-columns: repeat(3, 1fr); gap: 5px; padding: 10px;">
            ${Array.from({ length: 2026 - 1950 + 1 }, (_, i) => 2026 - i).map(ano => `
                <div class="cap-btn year-btn${ano < 2000 && !permiteHistoricas ? ' is-locked' : ''}" data-ano="${ano}">${ano < 2000 && !permiteHistoricas ? '<i class="fa-solid fa-lock"></i> ' : ''}${ano}</div>
            `).join('')}
        </div>
    `;
    ...
    container.querySelectorAll('.year-btn').forEach(btn => {
        btn.onclick = () => {
            if (Number(btn.dataset.ano) < 2000 && !permiteHistoricas) {
                mostrarBloqueioPublicacoesHistoricas(container);
                return;
            }
            listarEdicoesPorAno(pasta, btn.dataset.ano, container);
        };
    });
}
...
export async function listarEdicoesPorAno(pasta, ano, container) {
    if (['sentinelas', 'despertai'].includes(pasta) && Number(ano) < 2000) {
        const permitido = await podeAcederPublicacoesHistoricas();
        if (!permitido) {
            container.innerHTML = '';
            mostrarBloqueioPublicacoesHistoricas(container);
            return;
    ...
    const pastaReal = pasta === 'sentinelas' ? 'w' : (pasta === 'despertai' ? 'g' : pasta);
    ...
    const verificacoes = possiveis.map(async (f) => {
        const caminho = (['sentinelas', 'despertai', 'mwb'].includes(pasta))
...
export async function carregarPublicacaoJson(caminho, container, pastaOrigem, anoOrigem) {
    const caminhoLivro = caminho.match(/data\/livros\/([^/]+)\.json$/);
    const caminhoPublicacao = caminho.match(/data\/publicacoes\/([^/]+)\/(\d{4})\//);
    const dadosFonte = caminhoLivro
        ? { contexto: 'livro', sigla: caminhoLivro[1] }
        : caminhoPublicacao
            ? { contexto: 'publicacao', sigla: caminhoPublicacao[1], ano: caminhoPublicacao[2] }
            : {};

    if (await bloquearConteudoHistorico(dadosFonte, container)) return;
