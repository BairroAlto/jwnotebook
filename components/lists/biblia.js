...
async function carregarVersiculos(livro, capNum, container) {
    ...
    const pedidoAtual = ++pedidoCapituloAtual;
    ...
    try {
        ...
        if (pedidoAtual !== pedidoCapituloAtual || containerBibliaAtivo !== container) return;

        // Mostra o capítulo logo que o JSON chega. Os sublinhados entram depois,
        // sem bloquear a leitura nem manter o spinner visível.
        renderizarVersiculosDoCapitulo(container, livro, capNum, versiculosObj, new Map());

        if (window.NotaBookUserPrefs?.sublinhadosBibliaLists === true) {
            carregarSublinhadosDoCapitulo(livro.nome, capNum)
                .then(sublinhadosPorVersiculo => {
                    if (pedidoAtual !== pedidoCapituloAtual || containerBibliaAtivo !== container) return;
                    const scrollTop = container.querySelector('#biblia-scroll')?.scrollTop || 0;
                    renderizarVersiculosDoCapitulo(container, livro, capNum, versiculosObj, sublinhadosPorVersiculo);
                    container.querySelector('#biblia-scroll').scrollTop = scrollTop;
                })
                .catch(erro => console.warn('[BIBLIA] Sublinhados indisponíveis:', erro));
        }
...
function renderizarVersiculosDoCapitulo(container, livro, capNum, versiculosObj, sublinhadosPorVersiculo) {
    // Nota: o clique do #btn-voltar-caps é gerido pelo document.addEventListener acima
    container.innerHTML = `
    ...
    Object.keys(versiculosObj).forEach(vNum => {
        ...
        const textoRenderizado = renderizarSublinhados(texto, sublinhadosPorVersiculo.get(String(vNum)) || []);
