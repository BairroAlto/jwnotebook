...
unsubFontes = onSnapshot(currentRef, (docSnap) => {
    ...
    const linksAtivos = (fontes.Links || []).filter(l => l.estado === "on")
    ...
    const codexAtivos = (fontes.codex || []).filter(c => c.estado === "on")
        .sort((a,b) => (b.favorito === 'sim' ? 1 : 0) - (a.favorito === 'sim' ? 1 : 0));
    ...
    if (btnGroup) {
        ...
        btnDel.onclick = async (e) => {
            ...
            if (await confirmarAcaoBiblia("Remover Fonte?", "Desejas ocultar este link?")) {
                const novaLista = listaOriginal.map(l => l.timestamp === link.timestamp ? {...l, estado: "off"} : l);
...
export function abrirPopupFontesBiblia(dadosEdicao = null) {
    ...
    tabCodex.onclick = () => {
        ...
        abrirPesquisaCodex(async (dadosCodex) => {
            ...
            const novoItem = { ...dadosCodex, id: crypto.randomUUID(), userId: currentUid, timestamp: new Date().toISOString(), estado: "on", favorito: "nao", partilha: "off" };
