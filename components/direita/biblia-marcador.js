...
export async function abrirPopupMarcadores(verseInfo, db, auth) {
    ...
    if (!overlay || !container || !titulo) return;
    ...
    overlay.dataset.bookReadonly = readOnlyBook ? 'true' : 'false';

    const btnNovo = document.getElementById('btn-nova-ancora-trigger');
    const btnEditar = document.getElementById('btn-edit-ancoras-toggle');
    if (btnNovo) btnNovo.style.display = readOnlyBook ? 'none' : '';
    if (btnEditar) btnEditar.style.display = readOnlyBook ? 'none' : '';
    if (!readOnlyBook) {
        btnNovo.onclick = () => abrirFormMarcador();
        btnEditar.onclick = () => {
...
async function renderizarLista(lista) {
    const container = document.getElementById('lista-ancoras-container');
    const readOnlyBook = document.getElementById('popup-ancora-lista-overlay')?.dataset.bookReadonly === 'true';
    if (!container) return;
    container.innerHTML = "";
    ...
    lista.forEach(m => {
        ...
        div.innerHTML = `
            <div style="display:flex; align-items:center; gap:12px; overflow:hidden;">
                <input type="checkbox" ${isSelected ? 'checked' : ''} style="cursor:pointer; width:16px; height:16px;" ${(modoEdicaoMarcador || readOnlyBook) ? 'disabled' : ''}>
                <span style="font-size:13px; color:white; font-weight:500;">${m.nome}</span>
            </div>
            ${modoEdicaoMarcador && !readOnlyBook ? '<i class="fa-solid fa-pen-to-square btn-edit-sub" style="color:var(--primary); cursor:pointer; padding:5px;"></i>' : ''}
        `;

        if (readOnlyBook) {
            div.style.cursor = 'default';
        } else if (!modoEdicaoMarcador) {
...
async function vincularVersiculoMarcador(marcador, vincular) {
    ...
    if (!snapVerse.empty) {
        await updateDoc(snapVerse.docs[0].ref, { marcador: aindaTemMarcadores ? "sim" : "nao" });
    }
}
