...
export const AIInteraction = {
    ...
    initScrollSpy: (caixasAtivas, onHighlight) => {
        ...
        editor.addEventListener('scroll', () => {
            ...
            if (frameScrollAI !== null) return;
            frameScrollAI = requestAnimationFrame(() => {
                frameScrollAI = null;
                ...
                if (scrollPos < 50 && caixasAtivas[0]) {
                    onHighlight(caixasAtivas[0].id, { deslocarLista: false });
                    return;
                }
                ...
                if (maisProximo) onHighlight(maisProximo, { deslocarLista: false });
            });
        }, { passive: true });
     ...
     */
    aplicarDestaqueUI: (id, { deslocarLista = true } = {}) => {
        if (!id || AIInteraction.idCardAtivo === id) return;

        const cardAnterior = document.querySelector('#ai-blocks-list .indice-card.active');
        cardAnterior?.classList.remove('active');

        const card = document.getElementById(`ai-nav-${id}`);
        if (card) {
            AIInteraction.idCardAtivo = id;
