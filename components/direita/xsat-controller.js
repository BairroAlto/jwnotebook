...
function prepararFonteXSatBiblia() {
...
window.saltarParaFonteSat = (index, canalId) => {
 ...
 // 🕵️ LOG DE CLIQUE
    ...
    jumpInProgress = true;
    const fonteBibliaAberta = prepararFonteXSatBiblia();
    ...
    if (!fonteBibliaAberta && isMobileViewport() && typeof window.abrirFonteXSatMobile === 'function') {
        prepararDestinoListsNoMobile();
    }
    else if (!fonteBibliaAberta && btnLists) btnLists.click();
    else if (!fonteBibliaAberta && officeListsTab) officeListsTab.click();
    setTimeout(() => {
        import('../lists/bridge-main.js').then(m => {
            m.abrirReferenciaDireta(item.bridge);
            setTimeout(() => { jumpInProgress = false; }, 2500);
        }).catch(error => {
            console.error('[X-SAT] Não foi possível abrir a fonte.', error);
