...
let _SATELLITE_HISTORICAL_ACCESS = null;

async function podePesquisarPublicacoesHistoricas() {
    if (_SATELLITE_HISTORICAL_ACCESS !== null) return _SATELLITE_HISTORICAL_ACCESS;
    try {
        _SATELLITE_HISTORICAL_ACCESS = await obterAcessoFerramenta(window.auth, 'publicacoes_historicas');
    } catch (erro) {
        console.error('[X-SAT] Não foi possível verificar o acesso às publicações históricas:', erro);
        _SATELLITE_HISTORICAL_ACCESS = false;
    }
    return _SATELLITE_HISTORICAL_ACCESS;
}
...
export async function processarPesquisaSat(textoBruto, canalId) {
    ...
    const podePesquisarHistoricas = await podePesquisarPublicacoesHistoricas();
    ...
    for (const sigla in SIGLAS_PUBS) {
        if (!['w', 'g', 'mwb'].includes(sigla)) continue;
        ...
        for (const ano of ANOS_DISPONIVEIS) {
            ...
            const eHistorico = Number(ano) < 2000;
            if (eHistorico && !podePesquisarHistoricas) continue;
            if (!eHistorico && ano < 2020) continue;
