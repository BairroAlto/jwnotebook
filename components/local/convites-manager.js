...
let unsubscribeConvitesEntrada = null;
...
export function vigiarConvitesEntrada(db, auth) {
    ...
    if (unsubscribeConvitesEntrada) unsubscribeConvitesEntrada();
    unsubscribeConvitesEntrada = onSnapshot(q, (snapshot) => {
...
async function processarFluxoAceitacao(idOut, data, db, auth) {
    ...
    try {
        ...
        if (!(await verificarLimiteCaixas(auth, [], caixasRecebidas.length))) return;
