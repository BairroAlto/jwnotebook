...
function renderSeletor(container, docRef) {
    ...
    window.criarAnotacaoEspecial = async (tipo) => {
        if (['contentor', 'questao'].includes(tipo) && window.auth?.currentUser) {
            try {
                const permitido = await obterAcessoFerramenta(window.auth, tipo);
                if (!permitido) {
                    window.alert(`A ferramenta ${tipo === 'questao' ? 'Questão' : 'Contentor'} não está disponível no teu plano.`);
                    return;
                }
            } catch (erro) {
