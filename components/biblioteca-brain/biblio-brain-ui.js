...
import { bloquearConteudoHistorico, eConteudoHistorico, podeAbrirConteudoHistorico, mostrarBloqueioConteudoHistorico } from '../billing/historical-content.js';
...
export async function abrirEstudoNoBrain(estudo) {
    ...
    if (btnSat) {
        btnSat.onclick = async () => {
            ...
            if (eConteudoHistorico(estudo) && !(await podeAbrirConteudoHistorico())) {
                mostrarBloqueioConteudoHistorico(scrollArea);
    ...
    if (btnAI) {
        btnAI.onclick = async () => {
            ...
            if (eConteudoHistorico(estudo) && !(await podeAbrirConteudoHistorico())) {
                mostrarBloqueioConteudoHistorico(scrollArea);
                btnAI.classList.remove('fa-bounce');
                return;
            }
...
async function recuperarTextoDoRepositorio(estudo) {
    try {
        if (await bloquearConteudoHistorico(estudo, document.getElementById('biblio-dynamic-content'))) return null;
