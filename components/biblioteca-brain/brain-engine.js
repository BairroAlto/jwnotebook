...
export async function estudarReferencia(dados) {
    ...
    // 1. NORMALIZAÇÃO DE DADOS (Prevenção de duplicados por erro de digitação)
    ...
    console.log(`%cBridge: 🌉 Estudando: ${refLimpa} | §${seqLimpa}`, "color: #818cf8; font-weight: bold;");

    try {
        // 2. PREPARAR DADOS TÉCNICOS (Ano, Mês, Sigla) via Processadores
        const mapeamentoFake = { oque: dados.oque || "parágrafo", sequencia: [seqLimpa] };
        ...
        if (snap.empty) {
            // ========================================================
            // ✨ CENÁRIO: CRIANDO FICHA MESTRE INÉDITA
            // ========================================================
            console.log("%c✨ Bridge: Criando ficha mestre inédita.", "color: #34d399;");
            
            // Limpeza de campos temporários do processador
            ...
            const novaFicha = {
                ...
                Fontes: { Links: [], codex: [] },
                Dossie: { mica: {}, Apto: [] },
                // 🚀 INJETAR CAIXA AUTOMÁTICA (ANOTAÇÃO ESPECIAL)
                anotacaoEspecial: { 
                    id: crypto.randomUUID(), 
                    tipo: "questao", 
                    titulo: tituloLimpo, // O título do conteúdo (ou subtema) vira o título da questão
                    conteudo: "", 
                    estado: "on", 
                    foco: "original" 
                }
        ...
        console.error("❌ [BRIDGE-ERROR] Erro na transição para a Biblioteca:", error);
    }
}
