...
function obterPainel(tipo) {
    if (tipo === 'left') return document.getElementById('area-esquerda');
    return document.getElementById('area-direita') || document.getElementById('bible-right-col');
}
...
function prepararPainelFonte() {
    ...
    let lista = document.getElementById('lista-lists');
    if (!lista) {
...
function abrirFonteXSatMobile() {
    ...
    document.getElementById('bible-right-col')?.classList.add('source-open');
...
function fecharFonteXSatMobile(opcoes = {}) {
    ...
    const btnSource = document.getElementById('btn-source');
    document.getElementById('bible-right-col')?.classList.remove('source-open');
...
function abrir(tipo) {
    ...
    if (tipo === 'left') {
        ...
        if (!folhaJaAberta && !painel.style.height) {
            ...
            painel.style.height = Number.isFinite(pctGuardada)
                ? pctGuardada + 'vh'
                : (painel.id === 'bible-right-col' ? '88vh' : '68vh');
        }
        estado.right = 'open';
    }
