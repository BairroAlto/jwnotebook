 ...
 * ==========================================
 */
function obterMetadadosCodex(item = {}) {
    const referencia = String(item.referencia || "").trim();
    const sequencia = String(item.sequencia || "").trim();
    return {
        idBiblioteca: currentDocRef?.id || null,
        ...(referencia && sequencia ? { referenciacodex: [referencia, sequencia] } : {})
    };
}
async function handlePlusClick(event) {
    ...
    // A Bíblia tem um handler próprio (biblia-puzzle.js), que já recebe
    // explicitamente "simples" ou "conectora". Não duplicar essa acção aqui.
    if (event?.type === 'bible:adicionarTexto') return;
    ...
    try {
        if (escolha === 'simples') {
            ...
            const novo = { 
                id: novoId, 
                userId: currentUid,
                ...obterMetadadosCodex(cacheValido),
            ...
            if (!(await verificarLimiteCaixas(window.auth, caixasAtuaisDoc, 1))) {
                travaDuploClique = false;
                estaAEscrever = false;
                return;
            }
...
function reconstruirPuzzleUI(container, docRef, db, auth) {
    ...
    // Uma actualização live enquanto o utilizador escreve não deve substituir
    // o campo activo: em mobile isso fecha o teclado virtual.
    if (!container.__puzzleFocusOutHandler) {
        container.__puzzleFocusOutHandler = () => {
            setTimeout(() => {
                const contexto = container.__puzzleFocusOutContext;
                if (container.__puzzleRenderPendente && !editorEmFoco() && contexto) {
                    reconstruirPuzzleUI(container, contexto.docRef, contexto.db, contexto.auth);
                }
            }, 0);
        };
        container.addEventListener('focusout', container.__puzzleFocusOutHandler);
    }
    container.__puzzleFocusOutContext = { docRef, db, auth };
    ...
    if (assinatura === ultimoJsonRenderizado) {
        container.__puzzleRenderPendente = false;
        return;
    }

    if (editorEmFoco()) {
        container.__puzzleRenderPendente = true;
        return;
    }
...
function renderCaixaConectoraDirecta(c, index, todos, docRef) {
    ...
    const agendarSalvar = () => {
        ...
        // Mantém o cache local alinhado com o campo enquanto a gravação
        // atrasada não termina, para evitar perder texto ao sair do campo.
        const caixaLocal = (dadosAtuais?.caixas || []).find(
            item => typeof item === 'object' && item.id === c.id
        );
        if (caixaLocal) {
            if (inputTit) caixaLocal.titulo = inputTit.value;
            if (txtArea) caixaLocal.conteudo = txtArea.value;
        }
