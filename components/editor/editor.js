        ...
        btnBusca.onclick = async () => {
            ...
            try {
                ...
                if (!resultados || resultados.length === 0) {
                    status.innerHTML = `<span style="color:#f87171;">❌ Nenhuma nota encontrada com esse significado.</span>`;
                } else {
                    ...
                    resultados.forEach(nota => {
                        ...
                        card.style.cssText = `background: rgba(99, 102, 241, 0.08); border: 1px solid rgba(99, 102, 241, 0.2); border-left: 4px solid var(--primary); margin-bottom: 8px; padding: 12px 15px; cursor: pointer; display: flex; flex-direction: column; gap: 4px; transition: 0.2s;`;
                        card.innerHTML = `
                            <div style="display:flex; justify-content:space-between; align-items:center; width:100%; pointer-events:none;">
                                <div style="display:flex; align-items:center; gap:10px; overflow:hidden;">
                                    ...
                                    <span style="font-weight:700; color:white; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; font-size:13px;">${nota.title}</span>
                                </div>
                                <i class="fa-solid fa-arrow-right-to-bracket" style="opacity:0.3; font-size:12px;"></i>
                            </div>
                            ${nota.snippet ? `<div style="font-size:11px; color:var(--text-muted); padding-left:22px; font-style: italic; opacity:0.8; pointer-events:none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">"${nota.snippet}..."</div>` : ''}
                        `;
                        card.onclick = async () => {
            ...
            } catch (err) { status.innerHTML = `<span style="color:#f87171;">Erro na ligação ao Nexo.</span>`; }
