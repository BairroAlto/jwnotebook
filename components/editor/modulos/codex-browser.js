...
async function carregarFicheiroEProcessar(url) {
    ...
    const indicePublicacoes = caminhoAtual.indexOf('publicacoes');
    const dadosFonte = caminhoAtual.includes('livros')
        ? { contexto: 'livro', sigla: caminhoAtual[caminhoAtual.length - 1].replace('.json', '') }
        : indicePublicacoes !== -1
            ? {
                contexto: 'publicacao',
                sigla: caminhoAtual[indicePublicacoes + 1],
                ano: caminhoAtual.find(parte => /^\d{4}$/.test(parte)) || ''
            }
            : {};

    if (await bloquearConteudoHistorico(dadosFonte, lista)) return;
...
function renderizarNivelRaiz() {
    ...
    lista.innerHTML = `
        ...
        <div class="menu-item-list" onclick="window.codexNavegar('publicacoes/g')"><i class="fa-solid fa-newspaper"></i> Despertai!</div>
