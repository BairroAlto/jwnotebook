...
async function permitirAbaFirmamento(targetId) {
    const featureKey = targetId === 'destaques'
        ? 'personalizacao_firmamento_destaques'
        : 'personalizacao_firmamento_colorir';
    return exigirAcessoFerramenta(
        window.auth,
        featureKey,
        'Esta aba do Centro de Personalização requer o plano definido pelo administrador.'
    );
}

export async function abrirPaletaFirmamento(caixa, onAtualizar, abaInicial = 'colorir') {
    if (!(await permitirAbaFirmamento(abaInicial))) abaInicial = 'colorir';
    ...
    tabs.forEach(tab => {
        tab.onclick = async () => {
            if (await permitirAbaFirmamento(tab.dataset.firmamentoTab)) {
                selecionarTab(tab.dataset.firmamentoTab);
            }
        };
    });
