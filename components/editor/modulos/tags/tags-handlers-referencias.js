...
// Gatilho auxiliar para poupar código
...
/**
 * ADICIONA UMA NOVA REFERÊNCIA (COMPLETA OU LINK DIRETO)
 */
export function adicionarReferencia(ctx, tipo = "completa") {
    ...
    // 2. Atualiza a UI do próprio popup
    renderizarCards(ctx);
    
    // 3. GATILHO LIVE: Avisa a coluna da direita para atualizar a aba Fontes
   dispararUpdateEyeFontes();
}

/**
 * ATUALIZA CAMPOS DE UMA REFERÊNCIA EXISTENTE
 */
export function updateRef(id, campo, valor, ctx) {
    ...
    if (idx !== -1) {
        ...
        // 2. GATILHO LIVE: Reflete a escrita em tempo real na coluna EYE
       dispararUpdateEyeFontes();
    }
}

/**
 * ELIMINA UMA REFERÊNCIA
 */
...
/**
 * FUNÇÃO DE AUXÍLIO PARA SINCRONIZAR A COLUNA DA DIREITA
 */
function dispararUpdateEyeFontes() {
    import('../../../direita/eye-fontes-nota.js').then(m => {
        // Passamos as caixas atuais da RAM para o motor do EYE
        m.carregarFontesGlobaisDaNota(window.caixasAtuais);
    });
}

/**
 * RENDERIZA OS CARDS DE REFERÊNCIA DENTRO DO POPUP DE TAGS
 */
    ...
    container.innerHTML = refs.map(ref => {
        ...
        <div class="ref-card" style="background: rgba(255,255,255,0.03); border: 1px solid var(--border-color); padding: 12px; border-radius: 8px; margin-bottom: 10px; position: relative;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 10px;">
                ${isCompleta ? 
                    `<input type="text" value="${ref.titulo}" oninput="window.updateRefManual('${ref.id}', 'titulo', this.value)" placeholder="Título..." style="flex:1; font-weight:700; background:transparent; border:none; color:white; outline:none; font-size:13px;">` 
                ...
                <i class="fa-solid fa-trash-can" onclick="window.removerRefManual('${ref.id}')" style="color:#f87171; cursor:pointer; font-size:11px; padding:5px;"></i>
            </div>
