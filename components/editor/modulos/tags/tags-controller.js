...
const TAG_TAB_FEATURES = {
    'tags-hub': 'conexoes_hub',
    'tags-neuronios': 'conexoes_neuronios',
    'tags-associar': 'conexoes_associar',
    'tags-topicos': 'conexoes_topicos',
    'tags-referencias': 'conexoes_referencias',
    'tags-glosa': 'conexoes_glosa',
    'tags-codex': 'conexoes_codex',
    'tags-files': 'conexoes_files'
};

async function permitirAbaTags(tab) {
    const target = tab?.getAttribute('data-target');
    const featureKey = TAG_TAB_FEATURES[target];
    if (!featureKey) return true;
    const nome = tab.querySelector('.tab-tags-label')?.textContent?.trim() || 'Esta aba';
...
export function iniciarSistemaTags(db, auth) {
    ...
    document.querySelectorAll('.tab-tags').forEach(tab => {
        tab.onclick = async (event) => {
            ...
            if (!(await permitirAbaTags(tab))) return;
...
export function abrirPopupTags(caixa, notaId, origemNota) {
    ...
    todasAbas.forEach(aba => {
        ...
        // Regra B: O Elevador apenas tem acesso ÃƒÆ’Ã‚Â  aba de TÃƒÆ’Ã‚Â³picos
            ...
            aba.style.display = ['tags-topicos', 'tags-files'].includes(target) ? 'flex' : 'none';
        } 
        else {
    ...
    montarPainelFicheiros(document.getElementById('tags-files-panel'), {
        noteId: notaMaeId,
        contextType: 'caixa',
        contextId: caixaAlvo.id,
        titulo: 'Ficheiros desta caixa'
    });
...
export async function abrirPopupTagsNota(notaId, db, auth) {
    ...
    document.getElementById('search-tags-nota-unified').value = "";
    document.getElementById('results-tags-nota-unified').innerHTML = "";
    document.getElementById('results-tags-nota-unified').style.display = 'none';
    ...
    // 3. PESQUISA UNIFICADA DE TOPICOS E SUBTOPICOS (NOTA)
    const inTopicos = document.getElementById('search-tags-nota-unified');
    inTopicos.oninput = async (e) => {
        const termo = e.target.value.toLowerCase();
        if(termo.length < 2) {
            const resultados = document.getElementById('results-tags-nota-unified');
        ...
        <div class="neuronio-result-item" onclick="window.${funcaoClique}('${item.docIdFirebase}', '${item.id}', '${item.nome}')">
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div style="display:flex; flex-direction:column;">
                    ...
                    <small style="font-size:9px; color:var(--text-muted); text-transform:uppercase;">${item.tipo === 'subtopico' ? 'Subtópico' : 'Tópico'}</small>
                </div>
            </div>
