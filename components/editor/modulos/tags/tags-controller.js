...
function configurarInputsPesquisa() {
    ...
    if(inCosmos) {
        inCosmos.oninput = async (e) => {
            ...
            const q = query(collection(dbRef, "Cosmo"), where("userId", "==", authRef.currentUser.uid), where("tipo", "==", "cosmos"), where("estado", "==", "ativo"));
    ...
    btnSim.onclick = async () => {
        ...
        try {
            ...
            if (cardParaRemover) {
                ...
                cardParaRemover.estado = "desativo";
...
window.mostrarListaRelacoes = async (lista, titulo) => {
    ...
    const listaAtiva = lista.filter(link => link.estado !== 'desativo');
