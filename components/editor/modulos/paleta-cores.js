...
export function abrirPaleta(caixaAlvo) {
    ...
    if (!dbReferencia && window.db) { dbReferencia = window.db; uidLogado = window.auth.currentUser.uid; }

    // RE-VINCULAR ABAS SEMPRE QUE ABRE (Garante funcionamento no Brain)
    vincularCliquesAbas();
    ...
    // 1. ATIVAR BOTÃO REMOVER COR
   if (btnRemover) {
    btnRemover.onclick = () => {
        // REMOVIDO e.stopPropagation() para permitir que o Brain escute o clique
        caixaParaColorir.destaques = "";
        if (typeof callbackAtualizarEditor === 'function') callbackAtualizarEditor();
        
        // Se não estivermos no Brain (editor normal), fechamos o popup
    ...
    // 2. ATIVAR BOTÃO FECHAR (X)
    if (btnFechar) {
        btnFechar.onclick = () => overlay.classList.remove('active');
    }

    // 3. RENDERIZAR DESTAQUES (CORES)
    ...
    if (listaDest) {
        ...
        CORES_BASE.forEach(corObj => {
            ...
            div.style.cssText = `display:flex; flex-direction:column; background:${isSel ? 'rgba(255,255,255,0.08)' : 'var(--bg-panel)'}; border:2px solid ${isSel ? corObj.code : 'transparent'}; border-radius:4px; overflow:hidden; position:relative; cursor:pointer;`;
            div.innerHTML = `
                ...
                <div class="click-area" style="display:flex; align-items:center; justify-content:space-between; padding:12px 10px;">
                    ...
                    <i class="fa-solid fa-pen btn-edit-cor" style="font-size:10px; opacity:0.5;"></i>
                </div>`;

           div.querySelector('.click-area').onclick = () => {
    // REMOVIDO e.stopPropagation()
    ...
    if (listaFoco) {
        listaFoco.innerHTML = "";
        const fUsar = (caixaParaColorir.tipo === 'subnota') ? FOCOS_SUBNOTA : (caixaParaColorir.tipo === 'questao') ? FOCOS_QUESTAO : (caixaParaColorir.tipo === 'raciocinio') ? FOCOS_RACIOCINIO : FOCOS_BASE;

        Object.entries(fUsar).forEach(([key, obj]) => {
            const isSelF = (caixaParaColorir.foco === key || (!caixaParaColorir.foco && key === "original"));
            const row = document.createElement("div");
            row.style.cssText = `display:flex; align-items:center; gap:12px; padding:12px; background:${isSelF ? 'rgba(255,255,255,0.05)' : 'var(--bg-panel)'}; border:2px solid ${isSelF ? obj.corForte : 'var(--border-color)'}; border-radius:4px; cursor:pointer; margin-bottom:8px;`;
    ...
    // 5. CONFIGURAR BOTÕES DO POPUP DE NOME (GUARDAR/CANCELAR)
    const btnGuardarNome = document.getElementById('btn-guardar-nome-cor');
    const btnCancelarNome = document.getElementById('btn-cancelar-nome-cor');
    if (btnGuardarNome && !btnGuardarNome.dataset.init) {
        btnGuardarNome.dataset.init = "true";
        btnGuardarNome.onclick = async () => {
            const n = document.getElementById('input-nome-cor').value.trim();
            if (!n) return;
            ...
            document.getElementById('popup-editar-cor-overlay').classList.remove('active');
            abrirPaleta(caixaParaColorir);
        };
        btnCancelarNome.onclick = () => {
            document.getElementById('popup-editar-cor-overlay').classList.remove('active');
