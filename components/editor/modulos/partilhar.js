 ...
 */
window.trocarAbaPartilha = async (elemento, origem) => {
    const featureKey = origem === 'Share' ? 'partilhar_share' : 'partilhar_local';
    const acesso = await exigirAcessoFerramenta(
        partilhaAuth || window.auth,
        featureKey,
        `${origem} em Partilhar Secção requer o plano definido pelo administrador.`
    );
    if (!acesso) return;
