...
function desenharEstruturaPastas(container) {
    ...
    const gavetas = Object.values(dadosArquivo.gavetas || {}).filter(g => g.estado !== 'off');
    ...
    if (navState.view === 'raiz') {
        ...
        if (!gaveta || gaveta.estado === 'off') return voltarParaRaiz();
        ...
        if (!prateleira || prateleira.estado === 'off') return voltarParaGaveta();
...
function abrirPopupForm(tipo, gId = null, dadosExistentes = null) {
    ...
    if (confirm(`Desejas mover esta ${tipo} para a reciclagem?`)) {
        ...
        if (tipo === 'gaveta') {
            arquivo.gavetas[dadosExistentes.id].estado = "off";
            arquivo.gavetas[dadosExistentes.id].timedelete = timestamp; // 🚀 Registo
        } else {
            arquivo.gavetas[gId].prateleiras[dadosExistentes.id].estado = "off";
...
function renderizarBlocosDoNivel(ids, target) {
    if (!target) return;
    const caixas = notaCache.caixas.filter(c => ids.includes(c.id) && c.estado === 'on');
...
function abrirSeletorVinculo(gId, pId) {
    ...
    const listaCaixas = notaCache.caixas.filter(c => c.estado === 'on');
