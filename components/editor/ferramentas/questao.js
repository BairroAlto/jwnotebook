 ...
 */
export function criarQuestaoVerde(caixa, onTextoAlterado, onApagar, onPaleta, onPartilhar, onMover, onTags, onAddAbaixo) {
    const caixaDiv = document.createElement("div");
    
    const keyFoco = caixa.foco || "original";
    const focoInfo = FOCOS_QUESTAO[keyFoco] || FOCOS_QUESTAO["original"];
   ...
   inputTitulo.addEventListener("input", (e) => {
    ...
    if (brainTit && caixa.referenciacodex) {
        const refCaixa = `${caixa.referenciacodex[0]}|${caixa.referenciacodex[1]}`;
        if (window._brainRefAtiva === refCaixa) brainTit.value = e.target.value;
    }
});
   ...
   corpo.addEventListener("input", (e) => {
    ...
    // Verificação de segurança: só vincula o clique do "+" se ele existir no DOM
