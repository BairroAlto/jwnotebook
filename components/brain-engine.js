    ...
    unsubLocal = onSnapshot(query(collection(db, "Local"), where("userId", "==", currentUid)), (snapshot) => {
        ...
        snapshot.forEach(docNota => {
            ...
            (d.caixas || []).forEach(c => {
                ...
                if (c.estado === "ativa" && (isCosmos || isBiblioteca)) {
...
export async function iniciarFontes(colecao, item, container, db, auth) {
    ...
    unsubDoc = onSnapshot(docRef, (snap) => {
        ...
        const links = (fontes.Links || []).filter(l => l.estado === "ativo")
        ...
        const codices = (fontes.codex || []).filter(c => c.estado === "ativo")
            .sort((a,b) => (b.favorito === 'sim' ? 1 : 0) - (a.favorito === 'sim' ? 1 : 0));
...
export async function estudarReferencia(dados) {
    ...
    // 1. FORMATAR DADOS
    let fichaLimpa = {};
    const mapeamentoFake = { oque: dados.oque, sequencia: [parseInt(dados.sequencia)] };
    const baseParaProcessador = { referencia: dados.rawRef, titulo: dados.tituloConteudo };
    ...
    // 2. BUSCA DE PRECISÃO (Garantir que os campos batem com as Rules)
    const q = query(
    ...
    if (snap.empty) {
        console.log("✨ Criando nova ficha mestre na Biblioteca...");
        
        // --- REMOÇÃO DO ID DO PROCESSADOR ---
        // Retiramos o 'id' que o processador criou para o Firebase gerar o seu
        const { id, groupId, ...dadosParaGravar } = fichaLimpa;
        ...
        // O ID real é o docRef.id (o curto)
        docFinal = { ...novaFicha, id: docRef.id }; 
        console.log("✅ Ficha criada com ID Firestore:", docRef.id);
    } else {
        console.log("📂 Ficha existente encontrada. Carregando...");
        const docSnap = snap.docs[0];
        // Garantir que usamos o ID do documento Firestore
        docFinal = { ...docSnap.data(), id: docSnap.id };
    }

    console.groupEnd();
    // Enviamos o objeto com o ID correto para a UI
    import('./biblio-brain-ui.js').then(m => m.abrirEstudoNoBrain(docFinal));
}