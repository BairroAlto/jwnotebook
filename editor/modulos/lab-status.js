...
export function atualizarIconeLab(modos) {
    ...
    const isDiario = lista.includes('diario');
    const isSocial = lista.includes('social');
    ...
    else if ([isPost, isArquivo, isDiario, isSocial].filter(Boolean).length >= 2) {
        classe = "fa-vials";
        cor = "#fbbf24";
    } 
    else if (isPost) {
